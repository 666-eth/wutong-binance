...
(function() {
  ...
  function createDraggablePanel() {
      ...
      <div style="padding:0 28px;margin-bottom:12px;">
        <div style="display:flex;gap:10px;flex-wrap:wrap;align-items:center;">
          ...
          <div style="display:flex;align-items:center;gap:8px;background:#f0fdf9;border:1px solid #ccf0e6;color:#0b806c;border-radius:10px;padding:6px 10px;">
            <span style="font-weight:600;">锁价</span>
            <span id="cex-lock-source" style="color:#00bfae;font-weight:bold;font-size:1.1em;">--</span>
          </div>
  ...
  function bindPanelEvents() {
      ...
      // 锁价来源：根据当前买/卖tab，显示成交记录中“对手方”的最新价
      function getActiveSide() {
          try {
              const buyActive = !!getActiveTabByText('买入');
              const sellActive = !!getActiveTabByText('卖出');
              // 更严格：优先检查aria-selected
              const active = document.querySelector('.bn-tab.bn-tab__buySell[aria-selected="true"]');
              if (active && active.textContent.trim() === '买入') return 'buy';
              if (active && active.textContent.trim() === '卖出') return 'sell';
      ...
      function findLatestOppositeTradePrice(side) {
          const opposite = side === 'buy' ? 'Sell' : 'Buy';
          // 1) 首选：根据颜色变量匹配对手方最新价格（多处UI会复用该变量）
          const selector = `div[style*="--color-${opposite}"], span[style*="--color-${opposite}"]`;
          const nodes = Array.from(document.querySelectorAll(selector))
            .filter(el => el && el.offsetParent !== null);
          for (const el of nodes) {
              const v = parseNumeric(el.textContent);
              if (v && v > 0) return v;
          }
          // 2) 兜底：从可能的成交记录区域中提取数字
          try {
              const tradeRoots = Array.from(document.querySelectorAll('[class*="trade" i], [class*="record" i], [class*="history" i]'))
                .filter(el => el && el.offsetParent !== null);
              for (const root of tradeRoots) {
                  const cand = Array.from(root.querySelectorAll('div,span'));
      ...
      function updateLockSource() {
          try {
              const side = getActiveSide();
              const v = findLatestOppositeTradePrice(side);
              const el = document.getElementById('cex-lock-source');
              if (!el) return;
              if (v && isFinite(v)) {
                  el.innerText = `${side === 'buy' ? '卖一' : '买一'}: ${Number(v).toFixed(8)}`;
              } else {
                  el.innerText = '--';
              }
          } catch (e) {}
      ...
      try {
          if (lockSourceInterval) clearInterval(lockSourceInterval);
          updateLockSource();
          lockSourceInterval = setInterval(updateLockSource, 1000);
      } catch (e) {}
      ...
      // 绑定买入/卖出tab点击后，稍延迟刷新“锁价来源”
      try {
          Array.from(document.querySelectorAll('.bn-tab.bn-tab__buySell')).forEach(tab => {
              tab.addEventListener('click', () => setTimeout(updateLockSource, 250));
          });
      } catch (e) {}
      // 快卖按钮：执行统一的“快卖”函数
      if (quickSellBtn) {
      ...
      for (let page = 1; page <= totalPages; page++) {
          ...
          for (const row of rows) {
              ...
              if (/\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}/.test(timeStr)) {
                  ...
                  // 将页面显示时间视为“本地时区时间”，统一换算为北京时间
                  const localDate = new Date(y, m - 1, day, hh, mm, ss);
                  const tzOffsetMin = localDate.getTimezoneOffset(); // 本地相对UTC的分钟（东八区为-480）
                  const diffToBeijingMin = 8 * 60 + tzOffsetMin; // 需要加到本地时间上的分钟差
                  let bjDate = new Date(localDate.getTime() + diffToBeijingMin * 60000);
                  // 北京时间日界：08:00
                  if (bjDate.getHours() < 8) {
                      bjDate = new Date(bjDate.getTime() - 24 * 3600000);
                  }
                  const bjMonth = bjDate.getMonth() + 1;
                  const bjDay = bjDate.getDate();
  ...
  async function clickHistoryFirstPage() {
      try {
          ...
          if (page1) { page1.click(); logit('统计结束！'); }
      } catch (e) {
          logit('点击历史委托第一页出错:', e);
      }
  }
  function showStatResultByTokenDate(tokenStat) {
      ...
      try {
          function getTodayKeyByBeijing8() {
              const now = new Date();
              const diffToBeijingMin = 8 * 60 + now.getTimezoneOffset();
              let bjNow = new Date(now.getTime() + diffToBeijingMin * 60000);
              if (bjNow.getHours() < 8) {
                  bjNow = new Date(bjNow.getTime() - 24 * 3600000);
              }
              const m = bjNow.getMonth() + 1;
              const d = bjNow.getDate();
              return `${m}-${d}日`;
          }
